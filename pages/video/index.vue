<template>
	<view>
		<view class="topbar">
			<view class="h86"></view>
			<scroll-view class="scroll-view" scroll-x :show-scrollbar="false" scroll-with-animation :scroll-left="scrollLeft">
				<view class="rel">
					<view class="item" v-for="(item, index) in navList" :class="{ active: curNav == item.id }" ref="item" :key="index" @click="switchNav(item, index, $event)">
		...
		<mescroll-uni ref="mescroll" top="86" :down="downOption" :up="upOption" @down="downCallback" @up="upCallback" class="video-mes">
